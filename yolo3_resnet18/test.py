...
test_imgs = io.ImageCollection('./img/*.jpg').files
...
predictZHUITI = resnet.ResNet18(num_classes=5)
predictZHUITI.load_state_dict(preVerti)
predictZHUIJIANPAN = resnet.ResNet18(num_classes=2)
...
for img in test_imgs:
    ...
    in_file.close()
    print('检测：'+ picname +'，MSE = ', MSE, '，诊断准确性Accuracy = %.2f' % (accuracy*100/len(pridict_rec_list)),"%")
...
print("共检测图片", count, '张，平均每张耗时:%.4f' % (sum(aver_time)/count), 's')
