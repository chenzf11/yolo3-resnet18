# 该代码用于对训练集(train)进行测试，无需任何调整，可以直接运行
...
test_imgs = io.ImageCollection('./脊柱疾病智能诊断/train/data/*.jpg').files
...
def getMSE(picname, pre_rec):
    in_file = open('./VOCdevkit/VOC/Annotations/%s.xml' % (picname), encoding='utf-8')
...
yolo = YOLO()
summary(yolo.net, input_size=[(3,256,256)], device='cpu')
...
for img in test_imgs:
    ...
    reallab = open('VOCdevkit/VOC/Annotations/%s.xml' % (picname), encoding='utf-8')
    ...
    for obj in root.iter('object'):
        ...
        if len(cls) > 3:  # 椎间盘
            ...
            disc = classes2[index]
            if cls in tmp_real_lab and disc == tmp_real_lab[cls]:
                accuracy += 1
...
print("共检测图片", count, '张，平均每张耗时:%.4f' % (sum(aver_time)/count), 's,', '训练集总定位均方误差：',all_mse/count,'训练集总诊断精度：%.2f'%(all_accu/count),'%')
