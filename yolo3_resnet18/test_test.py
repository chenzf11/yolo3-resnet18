# 该代码用于对测试集(test)进行测试，无需任何调整，可以直接运行
...
test_imgs = io.ImageCollection('./脊柱疾病智能诊断/test/data/*.jpg').files
...
print("共检测图片", count, '张，平均每张耗时:%.4f' % (sum(aver_time)/count), 's,', '测试集总定位均方误差：',all_mse/count,'测试集总诊断精度：%.2f'%(all_accu/count),'%')
