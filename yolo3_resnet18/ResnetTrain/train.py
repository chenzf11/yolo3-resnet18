...
# 使用自定义类
store_path = 'C:/Users/czf/Desktop/机器人/ResNet-18-cifar10/cut_picture/zhuijianpan/'#存储路径
split = 'train'
name = {'v1':0, 'v2':1, 'v3':2, 'v4':3, 'v5':4}
#split是train，生成训练数据集
...
# 模型定义-ResNet
zhuijianpan_resnet = ResNet18(num_classes=5).to(device)

# 定义损失函数和优化方式
criterion = nn.CrossEntropyLoss()  #损失函数为交叉熵，多用于多分类问题
optimizer = optim.SGD(zhuijianpan_resnet.parameters(), lr=LR, momentum=0.9, weight_decay=5e-4) #优化方式为mini-batch momentum-SGD，并采用L2正则化（权重衰减）
        ...
        with open("log.txt", "w")as f2:
            for epoch in range(pre_epoch, EPOCH):
                ...
                for i, data in enumerate(trainloader, 0):
                    ...
                    # forward + backward
                    outputs = zhuijianpan_resnet(inputs)
                ...
                with torch.no_grad():
                    ...
                    for data in testloader:
                        zhuijianpan_resnet.eval()
                        images, labels = data
                        images, labels = images.to(device), labels.to(device)
                        outputs = zhuijianpan_resnet(images)
                    ...
                    print('Saving model......')
                    torch.save(zhuijianpan_resnet.state_dict(), '%s/net_%03d.pth' % (args.outf, epoch + 1))
